...
unsigned long time1, time2, total_time, avarage_time;
unsigned long *times;

void setup() {
  Serial.begin(115200);
  uint64_t input = 0x2D4375747465723B;
...
uint64_t des(uint64_t input, uint64_t key, char mode) {
  ...
  for (i = 0; i < 64; i++) {
    ...
    inv_init_perm_res <<= 1;
    inv_init_perm_res |= (pre_output >> (64 - a_PI[i])) & LB64_MASK;
